---
name: Stock Ledger Module
overview: Design and apply a hash-chained, append-only Stock Ledger with externally anchored tamper-evidence, first-class lots, reservations, period close, and a synchronously-maintained Stock Balance read model in DB_LOCATIONS, enabling forward and backward recall trace with mass balance.
todos:
  - id: get-live-endpoint
    content: Obtain connection details for the live legacy system holding current stock data, since production and production_dev on 3.10.84.186 are both empty skeletons; the shadow-run cannot be designed without it
    status: pending
  - id: split-credentials
    content: Create a runtime DB user with SELECT/INSERT only on stock tables and no UPDATE, DELETE, DROP or TRIGGER; reserve gazebo_dev for migrations only
    status: pending
  - id: verify-trigger-grant
    content: Confirm CREATE TRIGGER works on DB_LOCATIONS under binlog ROW format using one throwaway trigger on a scratch table, then drop it
    status: pending
  - id: ddl-tables
    content: Create stock_lot, stock_lot_amendment, stock_entry, stock_chain_head, stock_chain_anchor, stock_genealogy, stock_balance, stock_reservation and stock_period matching the int PK / datetime(6) / decimal(16,6) / utf8mb4_unicode_ci conventions
    status: pending
  - id: ddl-triggers
    content: Create the stock_entry insert trigger (inline hash chain, period-close guard, future-date guard), the head-advance trigger, the immutability SIGNAL guards on stock_entry, stock_genealogy and stock_lot_amendment, and the negative-balance guard on stock_balance
    status: pending
  - id: unit-conversion
    content: Build stock_unit_conversion with global rules for grams and Kg plus product-specific rows derived from product_packaging, and make mass-bearing entries fail loudly when no factor resolves
    status: pending
  - id: chain-verify
    content: Write the chain verification, balance invariant, transfer balance and reservation queries; prove each detects a deliberately corrupted row
    status: pending
  - id: s3-anchor
    content: Implement the periodic head-hash publisher to S3 Object Lock in compliance mode under a write-only IAM role, plus the anchor verification job
    status: pending
  - id: django-app
    content: "Write the single nz-inventory stock Django app: models for all nine tables, migration stock/0001_stock_ledger with RunSQL and reverse_sql for triggers, Meta.constraints for CHECKs"
    status: pending
  - id: service-layer
    content: "Implement the write API: receipt, issue, transfer pair, production output and consumption with genealogy edges, count adjustment as delta, reversal, and authorised negative override; idempotency key on every mutation with duplicate-key treated as success"
    status: pending
  - id: balance-projector
    content: Update stock_balance synchronously in the same transaction as the ledger insert, and add the scheduled verifier that recomputes from the ledger and alarms on drift rather than being the primary path
    status: pending
  - id: reservations
    content: Implement stock_reservation lifecycle and the available-to-promise calculation of balance minus open reservations
    status: pending
  - id: trace-api
    content: Implement forward and backward recall trace over the recursive CTEs, returning mass balance and yield loss per output entry
    status: pending
  - id: replay-diff
    content: Restore the dump to a scratch schema, replay its 2938 movements through the new ledger, and gate on all 1002 legacy tblstockcache quantities matching stock_balance exactly
    status: pending
  - id: shadow-run
    content: Once the live endpoint is known, run the new ledger in parallel on live traffic and diff balances continuously; blocked on get-live-endpoint
    status: pending
  - id: backfill
    content: Map tblstockmovement to stock_lot, stock_entry and stock_genealogy and backfill; blocked on the product module being populated beyond its current 1 row
    status: pending
---
...
| Source | movements | cache rows | products | recipe tree |
|---|---|---|---|---|
| `production` (live server) | 0 | 0 | 39 | 47 |
| `production_dev` (live server) | 0 | 0 | 39 | 47 |
| `Dump20260720.sql` | 2,938 | 1,002 | 2,387 | 4,576 |
...
Delivery mirrors the recipe module: full DDL in this plan, applied to `DB_LOCATIONS`, with Django models and a `stock/0001_stock_ledger` migration using `RunSQL` plus `reverse_sql` for triggers and `Meta.constraints` for the `CHECK`s.
